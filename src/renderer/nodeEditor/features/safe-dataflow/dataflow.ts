...
> = {
  ...
  outputs: () => (keyof O)[];
  data(
    fetchInputs: () => Promise<Partial<{ [key in keyof I]: I[key][] }>>
  ): Promise<O> | O;
};
...
export class Dataflow<Schemes extends ClassicScheme> {
  ...
  public add<T extends Schemes["Node"]>(
    node: T,
    setup: DataflowNodeSetup<T, any, any>
  ) {
  ...
  >(
    ...
    const inputKeys = (keys ?? result.inputs()) as readonly string[];
    ...
    const consWithSourceData = await Promise.all(
      cons.map(async (c) => {
        return {
          c,
   ...
   */
  public async fetch<T extends Record<string, any>>(
    nodeId: NodeId,
    stack: Set<NodeId>
  ): Promise<T> {
    if (stack.has(nodeId)) {
      console.error(
        `Dataflow: Loop detected in node ${nodeId}. This may cause infinite recursion.`
      );
      return {} as T; // Return empty object to prevent infinite recursion
    }
    stack.add(nodeId);
    ...
    try {
      ...
      if (!outputKeys.every((key) => returningKeys.includes(key))) {
        throw new Error(
          `dataflow node "${nodeId}" doesn't return all of required properties. Expected "${outputKeys.join(
            '", "'
          )}". Got "${returningKeys.join('", "')}"`
        );
      }
