...
// Prediction stop reason
/**
 * @public
 * Represents the reason why a prediction stopped.
 */
export const LLMPredictionStopReason = Type.Union(
  [
  ...
  { description: "Reason why the prediction stopped." }
);
export type LLMPredictionStopReason = Static<typeof LLMPredictionStopReason>;

// Prediction statistics
/**
 * @public
 * Statistics for LLM predictions.
 */
export const LLMPredictionStats = Type.Object(
  {
    stopReason: LLMPredictionStopReason,
    tokensPerSecond: Type.Optional(
      Type.Number({
        description: "The average number of tokens predicted per second.",
      })
    ),
    numGpuLayers: Type.Optional(
      Type.Number({
        description: "The number of GPU layers used in the prediction.",
      })
    ),
    timeToFirstTokenSec: Type.Optional(
      Type.Number({
        description: "The time it took to predict the first token in seconds.",
      })
    ),
    promptTokensCount: Type.Optional(
      Type.Number({ description: "The number of tokens that were supplied." })
    ),
    predictedTokensCount: Type.Optional(
      Type.Number({ description: "The number of tokens that were predicted." })
    ),
    totalTokensCount: Type.Optional(
      Type.Number({
        description: "The total number of tokens (prompt + predicted).",
      })
    ),
    usedDraftModelKey: Type.Optional(
      Type.String({
        description:
          "The model key of the draft model used if speculative decoding.",
      })
    ),
    totalDraftTokensCount: Type.Optional(
      Type.Number({
        description: "Total number of tokens generated by the draft model.",
      })
    ),
    acceptedDraftTokensCount: Type.Optional(
      Type.Number({
        description: "Number of draft tokens accepted by the main model.",
      })
    ),
    rejectedDraftTokensCount: Type.Optional(
      Type.Number({
        description: "Number of draft tokens rejected by the main model.",
      })
    ),
    ignoredDraftTokensCount: Type.Optional(
      Type.Number({
        description: "Number of draft tokens ignored by the main model.",
      })
    ),
  },
  { description: "Statistics for LLM predictions." }
);
export type LLMPredictionStats = Static<typeof LLMPredictionStats>;
