...
export function TemplateSheet({ open, onCreateFromTemplate }: TemplateSheetProps) {
  const [sp, setSp] = useSearchParams();
  const isOpen = open ?? sp.get("templates") === "open";
  ...
  return (
    <>
      ...
      <div
        ref={overlayRef}
        className={cn(
          "fixed inset-0 z-30 bg-black/20 transition-opacity pointer-events-none",
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b">
          ...
          <button className="px-2 py-1 text-sm hover:opacity-80" onClick={handleClose}>
            Close
          </button>
        </div>
...
function TemplateCard({
  ...
  const handleDragStart = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (img) e.dataTransfer.setDragImage(img, Math.floor(img.width / 2), Math.floor(img.height / 2));
  };

  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: drag-and-drop card needs div with event handlers
    <div
      className="rounded border w-full bg-card hover:bg-card/80 transition p-2 space-y-2"
      draggable
    ...
    >
      <div className="aspect-video overflow-hidden rounded border bg-muted flex items-center justify-center">
        ...
        <img src={t.src} alt={t.title} className="object-cover w-full h-full" loading="lazy" />
      </div>
      <div className="space-y-1">
        <div className="text-sm font-medium leading-tight">{t.title}</div>
        <div className="text-[10px] uppercase tracking-wide text-muted-foreground">{t.type}</div>
      ...
      <div className="pt-1">
        <button
          className={cn(
            ...
            isPng ? "hover:bg-accent" : "opacity-50 cursor-not-allowed"
          )}
