...
・OpenAIや、ローカルLLMに通信を行うため、ブラウザツールより、デスクトップアプリのほうが都合が良い。
...
・history機能がある。
・急にアプリが落ちても復帰できるように、編集中に状態を保存しておく。
・未保存かどうかを管理する。（historyの状態やhashを使う）。アプリを落とすときに、未保存だと保存を促す。


・複数のファイルをタブで並べる。タブを選択すると、rete.jsのeditorの状態を切り替えて、表示を変える。
...
### UI、見た目
・スタイリッシュ、業務的。色はトリコロールカラーをアクセントに、赤、青(データフロー)、黄色(コントロールフロー)、背景は白系。最近ガンダムにあやかろう。
