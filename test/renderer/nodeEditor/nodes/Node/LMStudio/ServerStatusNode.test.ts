...
test('execute fetches status and forwards exec', async () => {
  ...
  electronApiService.getServerStatus.mockResolvedValueOnce({ status: 'success', data: info })
  const forward = vi.fn()
...
test('execute sets error status on failure', async () => {
  const node = createNode()
  const { electronApiService } = await import('renderer/features/services/appService') as any
  electronApiService.getServerStatus.mockResolvedValueOnce({ status: 'error', message: 'fail' })
  const forward = vi.fn()
  await node.execute('exec', forward)
