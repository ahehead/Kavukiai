...
test('execute unloads models', async () => {
  ...
  electronApiService.unloadAllModels.mockResolvedValueOnce({ status: 'success', data: 'ok' })
  const forward = vi.fn()
...
test('execute sets error status on failure', async () => {
  const node = createNode()
  const { electronApiService } = await import('renderer/features/services/appService') as any
  electronApiService.unloadAllModels.mockResolvedValueOnce({ status: 'error', message: 'fail' })
  const forward = vi.fn()
  await node.execute('exec', forward as any)
